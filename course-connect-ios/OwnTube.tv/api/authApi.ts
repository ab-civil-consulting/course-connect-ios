...
export class AuthApi extends AxiosInstanceBasedApi {
  ...
  /**
   * Register a new user on the instance
   *
   */
  async register(baseURL: string, data: RegisterRequestArgs): Promise<RegisterResponse> {
    try {
      const response = await this.instance.post("users/register", data, {
        baseURL: `https://${baseURL}/api/v1`,
        headers: {
